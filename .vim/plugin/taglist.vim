...
" php language
let s:tlist_def_php_settings = 'php;f:function;d:constant;c:class'
...
function! s:Netrw_Window_Close()
    ...
    let winnum = bufwinnr("NetrwTreeListing")
    if winnum == -1
      let check_cur_buf = buffer_name('%')
      let my_result = strpart(check_cur_buf, 0, 5)
      if my_result == 'Netrw'
        let winnum = bufwinnr('%')
      endif
    endif
...
function! s:Tlist_Window_Toggle()
    ...
    if project_winnum != -1
        let s:return = 1
        call s:Project_Window_Close()
    endif

    if exists("s:return") && s:return == 1
      let s:return = 0
      return
    endif
...
""    endif
    ...
    if curwinnum != winnum && curwinnum != project_winnum
      call s:Tlist_Exe_Cmd_No_Acmds(curwinnum . ' wincmd w')
    endif
