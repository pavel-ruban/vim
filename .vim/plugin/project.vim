...
"            silent! bnext
    ...
    function! s:GrepAll(recurse, lineno, pattern)
        let dname=Project_GetRgrepDname(a:lineno)
        let g:grep_path_custom=dname
        let pattern=input("GREP options and pattern: ")
        exec "Rgrep " . pattern . " *"
    endfunction ">>>
    " GetXXX Functions <<<
    ...
    " Project_GetAllFnames(recurse, lineno, separator) <<<
    "   Grep all files in a project, optionally recursively
    function! Project_GetRgrepDname(line)
